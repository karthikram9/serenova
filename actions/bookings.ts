...
export async function createBooking(formData: FormData): Promise<ActionResult<{ bookingId: string }>> {
  ...
  if (Number.isNaN(scheduledAt.getTime()) || scheduledAt <= new Date()) {
    return { error: 'Choose a future session time.' }
  }
  ...
  try {
    ...
    return { error: 'Unable to verify availability.' }
  }
...
export async function submitPaymentProof(formData: FormData): Promise<ActionResult> {
  ...
  const { data: booking } = await supabase
    .from('bookings')
    .select('id, client_id, booking_status, payment_status')
...
export async function approveBooking(formData: FormData): Promise<ActionResult> {
  ...
  if (!parsed.success) return { error: 'Invalid approval details.' }
  ...
  let query = supabase
    ...
    .eq('id', parsed.data.bookingId)
    .eq('booking_status', 'payment_submitted')
...
export async function getPaymentProofSignedUrl(
  ...
  const { data: proof } = await supabase
    .from('payment_proofs')
    .select('storage_path, client_id, bookings!inner(therapist_id)')
  ...
  if (!isAdmin && row.client_id !== user.id && booking.therapist_id !== user.id) {
